...
get_cluster_table_by_sample<-function(nfile) {
    ...
    cellDiveID=gsub(".*neighborGraph_","",nfile) %>% gsub("___.*","",.)
...
aggCellType=gsub(".*___","",args[1]) %>% gsub("_.rda","",.)

write_csv(tbl,cc("cellAggregationTable",aggCellType,substr(digest::digest(args),1,8),".csv.gz"))
